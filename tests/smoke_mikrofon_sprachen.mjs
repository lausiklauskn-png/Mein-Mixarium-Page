 ...
 *
 * EHRLICH VORWEG, damit niemand mehr erwartet als dasteht: diese Seite hat
 * heute KEIN Textfeld. Der Mikrofon-Code kam als Kopie aus family-project mit
 * und lief bisher ins Leere. Er wird trotzdem gepflegt, weil `enhanceBareInputs()`
 * eine öffentliche Fläche ist: sobald irgendwann ein Feld dazukommt, bekommt es
 * ein Mikrofon — und dann darf es nicht die alte Fassung sein. Die Probe geht
 * deshalb genau diesen Weg: sie hängt ein blankes Feld ein und lässt die Seite
 * es nachrüsten.
 *
 * Vorher stand in wireMic:
 ...
 * die Sprach-Proben rot.
 *
 * Lauf:  PW_CORE=<pfad>/playwright-core/index.js node tests/smoke_mikrofon_sprachen.mjs
 */
...
/* Der Spion liefert NICHT synchron aus start(): die Seite setzt ihre Anzeige
 * erst NACH `start()`. Käme das Ergebnis sofort, überschriebe die Seite die
 * Meldung im selben Wimpernschlag und die Probe wäre fälschlich rot. */
const SPION = `
  window.__gehoert = [];
...
// Hängt ein blankes Feld ein und lässt die Seite es nachrüsten — genau der
// Weg, den ein künftiges Feld dieser Seite gehen wird.
const FELD_ANLEGEN = `
  var wrap = document.createElement("div");
  wrap.style.cssText = "display:flex";
  var inp = document.createElement("input");
  inp.type = "text"; inp.id = "probefeld";
  wrap.appendChild(inp);
  document.body.appendChild(wrap);
  window.FP.enhanceBareInputs();
`;
...
async function seite(locale) {
  ...
  await page.evaluate(FELD_ANLEGEN);
  return { ctx, page, errors };
}

// 0) Die Ausgangslage ehrlich festhalten: ohne Feld kein Mikrofon — und dann
//    auch keine Sprachwahl. Ein Knopf, der nichts bedient, wäre schlimmer.
{
  ...
  const r = await page.evaluate(() => ({
    mics: document.querySelectorAll(".mic").length,
    picker: !!document.getElementById("mmpMicLang")
  }));
  ok("ohne Textfeld: kein Mikrofon, keine Sprachwahl", r.mics === 0 && r.picker === false, JSON.stringify(r));
  await ctx.close();
}
...
for (const [locale, erwartet] of [["de-DE","de-DE"],["ar-EG","ar-SA"],["ps-AF","ps-AF"],["ru-RU","ru-RU"],["ja-JP","de-DE"]]) {
  ...
  const r = await page.evaluate(async () => {
    const sel = document.getElementById("mmpMicLang");
    document.querySelector(".mic").click();
    await new Promise((x) => setTimeout(x, 60));
    return { anzahl: sel ? sel.options.length : 0,
             dir: document.getElementById("probefeld").getAttribute("dir"),
  ...
  if (locale === "de-DE") {
    ...
    ok("Feld liest nach Inhalt (dir=auto)", r.dir === "auto", String(r.dir));
    ok("keine Seiten-Fehler", errors.length === 0, errors[0] || "");
  }
  await ctx.close();
}

// 2) Umschalten wirkt ab dem nächsten Antippen.
{
...
//    stehen. Vorher tat `rec.onerror` nichts als das Leuchten abschalten.
{
  const { ctx, page } = await seite("de-DE");
  const r = await page.evaluate(async () => {
    const s = document.getElementById("mmpMicLang");
...
// 4) Der STILLE Fehlschlag (Klaus' Sichttest 2026-08-11): Paschtu kam als
//    „Salaam" in LATEINISCHEN Buchstaben zurück, ganz ohne Fehler. Eine
//    Fehlermeldung kann da nicht greifen — also wird die Schrift geprüft. Und
//    sie muss schweigen, wenn die Schrift stimmt, sonst ist sie nur lästig.
{
  const { ctx, page } = await seite("de-DE");
  const r = await page.evaluate(async () => {
    const s = document.getElementById("mmpMicLang");
    const mic = document.querySelector(".mic");
    const feld = document.getElementById("probefeld");
